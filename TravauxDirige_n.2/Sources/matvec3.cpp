// Produit matrice-vecteur vecteur par colonne
# include <cassert>
# include <vector>
# include <iostream>
# include <mpi.h>
...
Matrix::operator * ( const std::vector<double>& u ) const
{
    ...
    int n_loc = m_nrows/nbp;
    int additional_col = m_nrows%nbp && rank < m_nrows%nbp;
    int offset = (m_nrows%nbp != 0) * (rank < m_nrows%nbp ? rank : m_nrows%nbp);

    std::vector<double> v(m_nrows, 0.);
    for ( int i = rank*n_loc + offset; i < (rank+1)*n_loc + additional_col + offset; ++i ) {
        for ( int j = 0; j < m_ncols; ++j ) {
            v[i] += A(i,j)*u[j];
        }
    }
    MPI_Allreduce(MPI_IN_PLACE, v.data(), m_nrows, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    return v;
}
